...
var a = 10
incrementorByMemory(&a) // 11

/*:
　与这种做法类似的是使用 Swift 的 inout 关键字。我们在将变量传入 inout 参数的函数时，同样也使用 & 符号表示地址。不过区别是在函数体内部我们不需要处理指针类型，而是可以对参数直接进行操作。
*/
func incrementorByInout(inout num: Int) {
    num += 1
}

var b = 10
incrementorByInout(&b)  /// 虽然 & 在参数传递时表示的意义和 C 中一样，是某个“变量的地址”，但是在 Swift 中我们没有办法直接通过这个符号获取一个 UnsafePointer 的实例

var uInt: UInt = 33
func printPointer(pointer: UnsafeMutablePointer<UInt>) {
    print(pointer)
}
printPointer(&uInt)
...
var intPtr = UnsafeMutablePointer<Int>.alloc(1) /// 申请了 1 个 Int 大小的内存
intPtr.initialize(10)   /// 初始化
...
let str = unsafeBitCast(CFArrayGetValueAtIndex(arr, 0), CFString.self) /// ？？？Core Foundation Sring
str // "meow"
